---
name: vsdd-commit
description: Create a git commit for the current VSDD phase. Auto-generates a conventional commit message from state.json including phase, sprint, gate verdict, and affected bead IDs. Tags with vsdd/<feature>/phase-<id>.
---

## What
Creates a git commit atomically staging source, tests, `.vsdd/features/<name>/`, and evidence files. Auto-generates a conventional commit message from pipeline state.
...
3. **Build commit message**:
   ```
   vsdd(2b): my-feature - implementation (green phase)
   ...
   - .vsdd/features/my-feature/evidence/sprint-1-green-phase.log [added]
...
This command is for **manual** phase commits. The `vsdd-auto-commit.js` hook handles auto-commits (only when `VSDD_AUTO_COMMIT=true` and worktree is clean).

## Examples

```bash
/vsdd-commit
/vsdd-commit --message "custom note"    # append custom note to auto-generated message
/vsdd-commit --tag-only                 # tag without new commit (already committed manually)
```
