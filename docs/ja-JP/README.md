...
PreToolUseフックがフェーズ外のファイル書き込みを自動的にブロックする。開発者が誤って作業順序を飛ばすことを防ぐ。
...
| vsdd-adversary | **opus** | Read, Grep, Glob | 敵対的レビュー | 読み取り専用、毎回新鮮なコンテキスト |
| vsdd-verifier | sonnet | Read, Bash, Grep, Glob | 形式検証の調整 | 言語プロファイル対応 |
...
| standard | + agents, skills, contexts | 通常の開発作業 |
| strict | + hooks, scripts（ゲート強制あり） | 高保証作業、チーム開発 |
